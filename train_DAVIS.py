...
def load_fn_example(train_id,ps,height,width):
    data_dir_ip = '/Dataset/Davis_Train_set_lowlight_images/'
    data_dir_gt = '/Dataset/Davis_Train_set_gt_images/'
    
    
    i1 = cv2.imread(f'{data_dir_ip}{train_id:04}/0002.jpg', cv2.IMREAD_UNCHANGED)
    
    i2 = cv2.imread(f'{data_dir_ip}{train_id:04}/0003.jpg', cv2.IMREAD_UNCHANGED)
    i3 = cv2.imread(f'{data_dir_ip}{train_id:04}/0004.jpg', cv2.IMREAD_UNCHANGED)
    ...
    gt = cv2.imread(f'{data_dir_gt}{train_id:04}.jpg', cv2.IMREAD_UNCHANGED)
    #img_pad = cv2.copyMakeBorder(img, 16, 85, 16, 85, cv2.BORDER_CONSTANT, (0,0,0))
    gt= gt[:480,:854,:]
...
#
print("data loading123...")
for i in range(n):
    
    ip,gt = load_fn_example(i+1,ps,height,width)
...
#     para1= sum(p.numel() for p in model.parameters() if p.requires_grad)    
#     return print("Number of parameters", para1)
...
for epoch in range(num_epochs):
    ...
    for i, (input_data, target) in tqdm(enumerate(tr_lod)):
        ...
        with torch.cuda.amp.autocast(enabled=use_amp):
            output = model(ip)
            #print("oshape",output,target.shape)
    ...
    if epoch%save_freq==0:
        torch.save(model, os.path.join(save_dir, 'DAVIS_%03d.pth' % (epoch+1)))
torch.save(model.state_dict(), f'{save_dir}/DAVIS_{num_epochs}_batch_size_{batch_size}_.pth')
print('weight_saved')
