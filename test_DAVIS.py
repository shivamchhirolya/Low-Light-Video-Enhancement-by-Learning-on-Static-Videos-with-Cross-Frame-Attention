...
model  = torch.load('checkpoints/DAVIS_901.pth')
...
def test(model,device,name):
    ...
    in_files= sorted(glob.glob(f'/Dataset/Davis_Test_set_lowlight_images_cl/{name}/*.jpg'))
    with torch.no_grad():
        ...
        for i in range(2,len(in_files)-2):
            ...
            op=np.uint8(op)
            cv2.imwrite(f'{save_dir}/{i:04}.jpg', op)
            print(f'Results/{i:04}')


for m in range(1,31):
    name= f'{m:04}'
    save_dir = f'Results_DAVIS/{name}/'
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    test(model,device,name)
    #make_video(m,name, save_dir)
    print(f'{name} done')
...
def test2(model,device,filename):
    ...
    in_files= sorted(glob.glob(f'Dataset/Davis_Test_set_lowlight_images_cl/{filename:04}/*.jpg'))
    gt_files= sorted(glob.glob(f'Dataset/Davis_test_set_gt/{filename:04}/*.jpg'))
    ...
    with torch.no_grad():
        ...
        k=0
        for i in range(2,len(in_files)-2):
            gt = cv2.imread(gt_files[i-1], cv2.IMREAD_UNCHANGED)
            
            gt= gt[:384,:768,:]
            
        
            i1 = cv2.imread(in_files[i-2], cv2.IMREAD_UNCHANGED)
            i2 = cv2.imread(in_files[i-1], cv2.IMREAD_UNCHANGED)
            i3 = cv2.imread(in_files[i], cv2.IMREAD_UNCHANGED)
            i1= i1[:384,:768,:]
            i2= i2[:384,:768,:]
            i3= i3[:384,:768,:]
...
# In[4]:
print("testing started multi...")
overall_psnr_loss=0
overall_ssim_loss=0
for i in range(1,31):
    loss, ssim_loss= test2(model,device,i)
...
total_avg_psnr= overall_psnr_loss/30
total_avg_ssim= overall_ssim_loss/30
print("total avg psnr {:0.3f}".format(total_avg_psnr))
print("total avg ssim {:0.3f}".format(total_avg_ssim))
